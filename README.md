...
3桁で管理。先頭がレジ（会計1=1／会計2=2）、末尾2桁 `XX` は **会計1・会計2それぞれ独立に 1〜50 を循環**。50 の次は 1 に戻る。会計1と会計2で同じ XX（例: `102` と `202`）が同時に存在するのは許容（先頭の帯で区別できる）。

- 発番は DB のカウンタ行（レジ別）をロックして直列化し、**同一レジの同時発番でも番号が重複しない**。
- そのレジで**受け渡し完了していない使用中の番号はスキップ**（同一レジで番号が被らない）。1〜50 が全て使用中なら発番不可（409）。
- 例: 会計1→`101`,`102`,`103` … / 会計2→`201`,`202`,`203` …（各レジ独立）。
